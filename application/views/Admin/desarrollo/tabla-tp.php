<div class="row">
				<div class="col-sm-12">
					<div id="data-table-default_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
						...
						<tbody><?php if (empty($_SESSION['tp'])){
								echo "<tr><td valign='top' colspan='3' class='dataTables_empty'>No se encontron registros</td></tr>";
							}else{?>
						<?php foreach ($_SESSION['tp'] as $key => $p) : ?>
                            <tr class="gradeX odd" role="row">
								<td><?php echo $p->id ?></td>
								<td><?php echo $p->tipo ?></td>
								<td>
                                	<button type="button" class="btn btn-danger btn-icon btn-sm" onclick="eliminar('<?=base64_encode($p->id)?>', 'tp')" title="Eliminar"><i class="fas fa-trash-alt fa-fw"></i></button>
                            	</td> 
							</tr>
							<?php endforeach; }?>  
							</tbody>
