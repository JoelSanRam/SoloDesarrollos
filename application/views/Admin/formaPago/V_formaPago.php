...
<div class="content" id="content">
	<div class="row">
		...
		<div class="col-md-9">
			<a class="btn btn-success col-md-3" type="button" href="#modal-dialog" style="float: right;" data-toggle="modal" data-target="#modal-dialog"><i class="fas fa-lg fa-fw m-r-10 fa-plus-circle"></i>Nueva forma pago</a>
		</div>
	</div>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						<label for="">Nueva forma de pago: </label> 
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<label for="">Actualizar forma de pago: </label> 
	...
	<script>
		$("#panel-contenido").load('<?=base_url()?>Admin/C_formaPago/tabla');

		function insertarForma(){
                $.ajax({
                    type: "POST",
                    url: '<?=base_url()?>Admin/C_formaPago/insertarFormaPago',
			...
			function updateForma(){
                $.ajax({
                    type: "POST",
                    url: '<?=base_url()?>Admin/C_formaPago/ActualizarForma',
			...
			function deleteForma(id){
				...
				.then((willDelete) => {
  					if (willDelete) {
						$.get("<?=base_url()?>Admin/C_formaPago/borrar_forma/"+id, 
						function(data) {
							if(data == 1){
								$("#panel-contenido").load('<?=base_url()?>Admin/C_formaPago/tabla');
