...
<div class="content" id="content">
	<div class="row">
		...
		<div class="col-md-10">
			<a class="btn btn-success col-md-3" type="button" href="<?php echo base_url(); ?>desarrolladora/nueva" style="float: right;"><i class="fas fa-lg fa-fw m-r-10 fa-plus-circle"></i>Nueva desarrolladora</a>
		</div>
	</div>
	...
	<script>
		...
		function confirmarEliminar(key){
			swal({
				...
				.then((willDelete) => {
				if (willDelete) {
					eliminarDesarrolladora(key);
				} else {
					swal("Operación abortada");
				}
			});
		}
		function eliminarDesarrolladora(key){
			var formData = new FormData();
			formData.append('key', key);
			$.ajax({
                // la URL para la petición
                url : '<?=base_url()?>desarrolladora/eliminar',
                ...
                success : function(data) {
                    ...
                    if(data > 0){
						...
						$("#panel-contenido").load('<?=base_url()?>desarrolladora/tabla');
                    }else{
                        swal({
                                icon: 'error',
