...
<div class="content" id="content">
	<div class="row">
		...
		<div class="col-md-9">
			<a class="btn btn-success col-md-3" type="button" href="#modal-dialog" style="float: right;" data-toggle="modal" data-target="#modal-dialog"><i class="fas fa-lg fa-fw m-r-10 fa-plus-circle"></i>Nuevo servicio</a>
		</div>
	</div>
	...
	<script>
		$("#panel-contenido").load('<?=base_url()?>Admin/C_servicio/tabla');

		function insertarForma(){
                $.ajax({
                    type: "POST",
                    url: '<?=base_url()?>Admin/C_servicio/insertarFormaPago',
			...
			function updateForma(){
                $.ajax({
                    type: "POST",
                    url: '<?=base_url()?>Admin/C_servicio/ActualizarForma',
			...
			function deleteForma(id){
				...
				.then((willDelete) => {
  					if (willDelete) {
						$.get("<?=base_url()?>Admin/C_servicio/borrar_forma/"+id, 
						function(data) {
							if(data == 1){
								$("#panel-contenido").load('<?=base_url()?>Admin/C_servicio/tabla');
