					...
					<div id="data-table-default_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
						...
						<?php foreach ($tipo as $key => $p) : ?>
                            <tr class="gradeX odd" role="row">
								<td class="f-s-600 text-inverse sorting_1" tabindex="0" width="1%"><?php echo $p->Id_tipo ?></td>
								<td class="with-img" ><?php echo $p->Tipo ?></td>
								<td><?php echo $p->Nombre ?></td>
								<td>
									
                                	<button type="button" class="btn btn-grey btn-icon btn-sm"  title="Editar" onclick="editarModelo('<?=base64_encode($p->Id_tipo)?>')"><i class="fas fa-pencil-alt fa-fw"></i></button>
                                
                                <button type="button" class="btn btn-danger btn-icon btn-sm" onclick="confirmarEliminar('<?=base64_encode($p->Id_tipo)?>')" title="Eliminar"><i class="fas fa-trash-alt fa-fw"></i></button>
                            </td> 
							</tr>
							<?php endforeach; ?>  
							</tbody>
