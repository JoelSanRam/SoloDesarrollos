...
<div class="content" id="content">
	<div class="row">
		...
		<div class="col-md-10">
			<a class="btn btn-success col-md-4" type="button" href="<?=base_url();?>tipo/inmueble/nuevo" style="float: right;"><i class="fas fa-lg fa-fw m-r-10 fa-plus-circle"></i>Nueva tipo de inmueble</a>
		</div>
	</div>
	...
	<script>
		...
		function eliminarModelo(key){
			var formData = new FormData();
			formData.append('key', key);
			$.ajax({
                // la URL para la petición
                url : '<?=base_url()?>tipo/inmueble/eliminar',
                ...
                success : function(data) {
                    ...
                    if(data > 0){
						...
						$("#panel-contenido").load('<?=base_url()?>tipo/inmueble/tabla');
                    }else{
                        swal({
                                icon: 'error',
		...
		function editarModelo(key){
			$.redirect('<?=base_url()?>tipo/inmueble/modificar', {'key': key });
		}
	</script>
	<script>
        $("#panel-contenido").load('<?=base_url()?>tipo/inmueble/tabla');
