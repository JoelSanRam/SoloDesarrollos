...
<div class="content" id="content">
	<div class="row">
		...
		<div class="col-md-10">
			<a class="btn btn-success col-md-2" type="button" href="<?php echo base_url(); ?>modelo/nuevo" style="float: right;"><i class="fas fa-lg fa-fw m-r-10 fa-plus-circle"></i>Nueva modelo</a>
		</div>
	</div>
	...
	<script>
			...
			$.ajax({
                ...
                success : function(data) {
                    ...
                    if(data > 0){
						...
						$("#panel-contenido").load('<?=base_url()?>modelo/tabla');
                    }else{
                        swal({
                                icon: 'error',
		...
		function editarModelo(key){
			$.redirect('<?=base_url()?>modelo/modificar', {'key': key });
		}
	</script>
	<script>
        $("#panel-contenido").load('<?=base_url()?>modelo/tabla');
